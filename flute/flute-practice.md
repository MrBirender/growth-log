# Flute Practice

> Daily slot: 3:30 – 4:00 PM (30 minutes)
> Goal: build consistency first, skill second. 30 minutes every day beats 2 hours once a week.

---

## Current Level
Beginner. Can play all 7 swaras (Sa Re Ga Ma Pa Dha Ni Sa). Attempting simple songs. Notes still break at higher swaras — airflow not fully consistent yet.

## Current Focus
Clean tone on all 7 swaras + building song recognition by ear (Sathiya).

---

## How to use 30 minutes

A structured 30-minute session:

| Minutes | What to do |
|---|---|
| 0–5 | Long tones — play Sa and hold it as long as possible, focus on steady air |
| 5–20 | Sargam ascending and descending — slow, one note at a time, stop on any breaking note and repeat it 5 times |
| 20–28 | Play sargam at a comfortable pace without stopping — accept small breaks, keep going |
| 28–30 | Note which note broke most today (one line in the log) |

---

## Progress Log

*(One line per session: date + what you worked on + what was hard)*

| Date | Worked on | Hard part |
|---|---|---|
| 2026-05-20 | Sargam — first session, establishing habit | Notes breaking, airflow inconsistent |
| 2026-05-22 | Sa, Re, Ga, Ma — focused on first four notes | Working on clean tone, notes still breaking |
| 2026-05-22 | Short session — 5 min only | Consistency missed today |
| 2026-05-23 | Long notes — Sa Re Ga Ma, full 30 min | Steadier airflow, notes less broken than before |
| 2026-05-25 | All 7 swaras + Sathiya song (10 min) | First attempt at a real song |
| 2026-05-26 | Pa Dha Ni Sa Re Ga Ma ascending + descending — 30 times | Consistency across full range |
| 2026-05-27 | Full sargam 10 min + focused on Ma and higher Pa | Higher Pa still breaking — upper register airflow not steady |
| 2026-05-28 | Ma Pa focused practice + sargam ascending and descending | Consistency on Ma–Pa transition |
